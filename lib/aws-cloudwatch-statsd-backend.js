...
CloudwatchBackend.prototype.processKey = function(key) {
  ...
  var namespace = parts.length > 1 ? parts.splice(0, parts.length - 1).join("/") : null;
  var dimensions = [];
  ...
  if (name[0] == '[') {
    ...
    // node shenanigans...
    //var i = n.toString().matchAll(/(([^=]+=[^,\]]+)(,|\])?)/gm);
    var matches = n.toString().match(/([^=]+=[^,\]]+),?/gm);
...
CloudwatchBackend.prototype.batchSend = function(currentMetricsBatchMap) {

  for (var namespace in currentMetricsBatchMap) {
    var currentMetricsBatch = currentMetricsBatchMap[namespace]

    console.log('[batchSend] namespace: ' + namespace + ', len currentMetricsBatch: ' + currentMetricsBatch.length);
...
CloudwatchBackend.prototype.flush = function(timestamp, metrics) {
  ...
  for (key in counters) {
    ...
    var dimensions = this.config.processKeyForNamespace ? names.dimensions : [];

    currentCounterMetrics[namespace] = currentCounterMetrics[namespace] || []
  ...
  for (key in timers) {
    if (timers[key].length > 0) {
      ...
      currentTimerMetrics[namespace] = currentTimerMetrics[namespace] || []
  ...
  for (key in gauges) {
    ...
    currentGaugeMetrics[namespace] = currentGaugeMetrics[namespace] || []
  ...
  for (key in sets) {
    ...
    currentSetMetrics[namespace] = currentSetMetrics[namespace] || []
